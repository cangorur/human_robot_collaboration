...
class Human(GraspingRobot):
    ...
    def obj_reset(self):
        ...
        obj.worldPosition = [7.7, -2.2, 0.80] #real value of y is -2.1
...
#
    ...
    def grasping_attempt(self, distance):
        ...
        if (self.package_in_range(distance)):
            ...
            hand_l = scene.objects['IK_Target_Empty.L']
            obj = scene.objects['package1']
            obj.worldPosition = [(hand_r.worldPosition[0]+hand_l.worldPosition[0])/2.0, (hand_r.worldPosition[1]+hand_l.worldPosition[1])/2.0, (hand_r.worldPosition[2]+hand_l.worldPosition[2])/2.0]
    ...
    def package_fell_on_ground(self, height):
        scene = blenderapi.scene()
        obj = scene.objects['package1']
        vec = obj.worldPosition
        if (vec[2]<=height):
            return True
        return False
...
########################################
    ...
    def distance(self):
        ...
        vec1 = (hand_r.worldPosition+hand_l.worldPosition)/2.0
        vec2 = obj.worldPosition
