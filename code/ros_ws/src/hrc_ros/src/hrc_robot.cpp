...
ros::ServiceClient call_taskMang; // Calling the task manager service for status update
...
void initialize() {
	...
	ROS_INFO("Robot node is initialized.");
}

bool resetScenario(hrc_ros::ResetRobotROSRequest &req,
	...
	echo.on_message=[&](shared_ptr<WsServer::Connection> connection, shared_ptr<WsServer::Message> message) {
		...
		*send_stream << message_send;
		...
		if (robot_action != "-1"){
			...
			/* call action service*/
			ros::ServiceClient pointToObj = nh.serviceClient<std_srvs::Trigger>("/robot/point_to_obj");
			ros::ServiceClient grasp = nh.serviceClient<std_srvs::Trigger>("/robot/grasp");
			ros::ServiceClient cancelAction = nh.serviceClient<std_srvs::Trigger>("/robot/cancel_action");
			ros::ServiceClient planForGrasp = nh.serviceClient<std_srvs::Trigger>("/robot/planning_for_grasping");
		...
		if ((action_info_received && newstate_info_received) || initial_state_received){
			...
			call_taskMang.call(robotUpdateReq, robotUpdateRes);
		}
...
int main(int argc, char **argv) {
	...
	ros::init(argc, argv, "hrc_robot");

	ros::NodeHandle nh;

	// loop at 2Hz until the node is shut down
	// ros::Rate rate(0.2); // Server is opening a new thread so no need for ros rate
	call_taskMang = nh.serviceClient<hrc_ros::InformRobotToTaskMang>("/hrc_task_manager/robot_status_update");  // Client for the task manager service for the status update
	
	reset_scenario = nh.advertiseService("/hrc_robot/reset", &resetScenario);
