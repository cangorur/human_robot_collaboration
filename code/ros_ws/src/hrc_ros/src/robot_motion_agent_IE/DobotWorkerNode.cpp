...
void receiveTrayStatusCallback(const hrc_ros::SuccessStatusObserved &msg)
{
	return;
}
...
int main(int argc, char **argv) {
	...
	init_drop_locations();
  	ROS_INFO("Robot Motion Agent multi is ready - Dobot can be controlled now !");
		...
		//TODO: decreasing the number of threads? Only two?
  ...
  ros::Subscriber dobot_cancel_sub = nh.subscribe("/robot_motion_agent/dobot_cancel",1, cancelCallback);
	...
	ros::Subscriber traySensor_sub = nh.subscribe("/observation_agent/observedsuccess_status", 1000, receiveTrayStatusCallback);
