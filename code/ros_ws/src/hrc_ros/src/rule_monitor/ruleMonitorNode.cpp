	...
	*/
	...
	if (0) { //Option 0: Human is not aware if he did correctly or not, 1: Human is notifyed whether success or fail
		if( ( msg.subtask_success_status.compare("success") == 0 ) ) {
				...
				cout << normal << "You placed " <<  obj_colour << current_object_str << normal <<" object into " << container_colour << current_tray_str << normal << " container."<< endl;
				...
				cout << normal << "You placed " <<  obj_colour << current_object_str << normal <<" object into " << container_colour << current_tray_str << normal << " container; instead of "<< success_colour << success_tray_str << normal << " container." << endl;
...
bool display_task_rules_server(hrc_ros::DisplayTaskRuleRequest &req,hrc_ros::DisplayTaskRuleResponse &res){
	...
	if (current_task_set.all_set == true){ // rules are the same for all subtasks
		...
		if (subtask_type.compare("2_same_tray_order") ==0) { // regardless of colour the object always goes to same tray - sequence is reapeated after 5 subtasks
