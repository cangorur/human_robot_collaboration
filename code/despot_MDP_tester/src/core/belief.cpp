...
vector<pair<string, double>> ParticleBelief::Update(string new_state, int action, OBS_TYPE obs) {
	...
	//====== GET THE BELIEF DISTRIBUTION PROBABILITIES OF THE NEXT STATE. IN MDP IT IS NOT IMPORTANT IF PROVIDED MANUALLY ======//
	// THIS ROBOT MDP HAS NO BELIEF. DIRECT INPUTS AS THE NEXT STATES
	...
	// PRINT BELIEF DISTRIBUTION: This loop is just to print the current belief distribution of the robot before overwriting it manual states
	vector<State*> original_particles = particles_;
	for (int i = 0; i < original_particles.size(); i++) {
		if (original_particles[i]->weight != 0.0){
			original_particles[i]->weight = 1/double(count_all);
		}
	}
	map<string, double> original_pdf;
	for (int i = 0; i < original_particles.size(); i++) {
		original_pdf[original_particles[i]->text()] += original_particles[i]->weight;
	}

	vector<pair<string, double>> pairs = SortByValue(original_pdf);
	ostringstream oss;
	...
	// End PRINT BELIEF DISTRIBUTION //

	// HERE WE OVERWRITE THE MANUAL STATE'S BELIEF PROBABILITY BY 0.6 and reconstruct the pdf and pairs 
	for (int i = 0; i < particles_.size(); i++) {
		if (particles_[i]->text() == new_state){
			particles_[i]->weight = 0.6/double(count_newState);
		}
		else if (particles_[i]->weight != 0.0){
			particles_[i]->weight = 0.4/double(count_others);
		}
	}
