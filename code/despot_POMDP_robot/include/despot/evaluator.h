...
#include <despot/simple_websocket/client_ws.hpp>
#include <unistd.h>
#include <vector>
#include <string>
...
public:
	...
	void webSocketClient(int action, std::string belief_state, double reward, double discounted_reward);
	bool RunStep(int step, int round, int new_state, int manual_obs);
	bool RunInitial();

	virtual double EndRound() = 0; // Return total undiscounted reward for this round.
	virtual bool ExecuteAction(int sim_type, int new_state, int action, double& reward, OBS_TYPE& obs) = 0;
...
public:
	...
	void InitRound();
	double EndRound();
	bool ExecuteAction(int sim_type, int new_state, int action, double& reward, OBS_TYPE& obs);
	double End();
	void UpdateTimePerMove(double step_time);
};
