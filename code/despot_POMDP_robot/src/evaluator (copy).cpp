...
//==========================================================
bool Evaluator::websocketSend(int action) {
  ...
  client.on_open=[&]() {
      ...
      string message = std::to_string(action);

      *out_ << "Client: Opened connection" << endl;
      *out_ << "Client: Sending message: \"" << message << "\"" << endl;
...
//==========================================================
//==========================================================
std::vector<std::string> Evaluator::websocketReceive() {
	typedef SimpleWeb::SocketClient<SimpleWeb::WS> WsClient;

	std::vector<string> out;

	// receive observation from human
  ...
  client.on_message=[&](shared_ptr<WsClient::Message> message) {
      ...
      cout << "Client: Message received: \"" << message_receive << "\"" << endl;

			std::size_t index = message_receive.find(",");
    	string observation = message_receive.substr(0, index);
    	string state = message_receive.substr(index+1);

    	out.push_back(observation);
    	out.push_back(state);

/*
    	string observation = message_receive;
    	out.push_back(observation);
*/
...
bool Evaluator::RunStep(int step, int round) {
	...
	*out_ << "-----------------------------------Round " << round
							<< " Step " << step << "-----------------------------------"
							<< endl;
	*out_ << "Agent Took: a" << action << ", in the State: " << state_->text() << endl << endl;

	//== send the action to robot via websocket ==//
	Evaluator::websocketSend(action);
	...
	//== receive the next observation and state via websocket ==//
	//== ask every 2 seconds ==//
	usleep(2000000);
	std::vector<string> obs_state = Evaluator::websocketReceive();

	// get state from websocket
	string input = obs_state[1];	////

	//std::vector<string> obs_state = Evaluator::websocketReceive();

	int new_state;
	int sim_type = 0; //random state generation by default
	////string input = "";	////
	////getline(cin, input);	////
	// This code converts from string to number safely.
	stringstream stateString(input);
	if (stateString >> new_state){
		*out_ << "State is received from websocket: " << new_state << endl;
		sim_type = 2;
	}
	else{
	...
	// get obs from websocket
	input = obs_state[0];

	int manual_obs;
	// This code converts from string to number safely.
	stringstream obsString(input);
	if (obsString >> manual_obs){
		*out_ << "Observation is received from websocket: " << manual_obs << endl;
		sim_type = 2;
	}
	else{
		*out_ << "Assignment failed, random observation generation..." << endl;
	}
	//== ==//

	/*
	//== Setting the next state manually ==//
	*out_ << "Please enter the next state [int from 0 to max_state_numb,..."
			"Note that this is for rewarding the agent. Press any non-int key for autogen]: ";
	int new_state;
	int sim_type = 0; //random state generation by default
	string input = "";
	getline(cin, input);
	...
	stringstream stateString(input);
	if (stateString >> new_state){
		*out_ << "State is manually entered: " << new_state << endl;
		sim_type = 2;
	}
	else{
		*out_ << "Assignment failed, random state generation..." << endl;
	...
	//== Setting the observation manually ==//
	*out_ << "Please enter the observation [int from 0 to max_obs_numb. Press any non-int key for autogen]: ";
	...
	stringstream obsString(input);
	if (obsString >> manual_obs){
		*out_ << "Observation is manually entered: " << manual_obs << endl;
		sim_type = 2;
	}
	else{
		*out_ << "Assignment failed, random observation generation..." << endl;
	...
	*/
	...
	//TODO: observation has no effect in this ExecuteAction func. To be removed
	bool terminal = ExecuteAction(sim_type, new_state, action, reward, obs);
	obs = manual_obs;
	terminal = false; // TODO: we disabled the termination of the states for continuous testing
	*out_ << "=== RESULTS ===" << endl;
	*out_ << "- Resulting State:" << state_->text() << endl;
	ReportStepReward();
...
bool POMDPEvaluator::ExecuteAction(int sim_type, int new_state, int action, double& reward, OBS_TYPE& obs) {
	bool terminal = true;
	...
	if (sim_type == 0){ // simulator generates random(relevant) next states
		...
		terminal = model_->StepManual(*state_, random_num, new_state, action, reward, obs);
	}
