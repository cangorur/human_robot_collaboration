...
bool POMDPX::StepManual(State& s, double random_num, int new_state, int action, double& reward,
	OBS_TYPE& obs) const {
	POMDPXState& prev_state = static_cast<POMDPXState&>(s);
	POMDPXState curr_state = static_cast<POMDPXState&>(s);
	curr_state.vec_id[0] = new_state;

	reward = parser_->GetReward(prev_state.vec_id, curr_state.vec_id, action); // Prev state and curr state set in GetNextState
	//obs = parser_->GetObservation(curr_state.vec_id, action, random_num);
	
	//casting curr state to prev state
	prev_state = curr_state;
	
	return parser_->IsTerminalState(curr_state.vec_id);
...
public:
	...
	vector<pair<string, double>> Update(int action, OBS_TYPE obs) {
