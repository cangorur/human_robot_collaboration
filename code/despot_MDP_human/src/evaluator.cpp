...
void Evaluator::webSocketClient(int action, std::string state) {
    ...
    // Commented below to send the state name directly
    /*string state_ID = "";
    if (state_str == "task_assigned")
        state_ID = "0";
    else if (state_str == "success")
        state_ID = "1";
    else if (state_str == "failed_to_grasp")
        state_ID = "2";
    else if (state_str == "no_attention")
        state_ID = "3";
    else if (state_str == "evaluating")
        state_ID = "4";
    else if (state_str == "tired")
        state_ID = "5";
    ...
    */
    ...
    client.on_open=[&]() {
        ...
        *out_ << ">>> HUMAN MDP Client: Sending message: \"" << message << "\"" << endl;
    ...
    client.on_message=[&](shared_ptr<WsClient::Message> message) {
        //this crashes if we check for -1 and assign afterwards. not sure why
        string msg_received = message->string();
        if(msg_received != "-1"){
            m_new_state = msg_received;
            *out_ << "Client: received next state " << m_new_state << endl;
        }
...
bool Evaluator::RunInitial() {
    ...
    Evaluator::webSocketClient(action, state_->text()); // informing about the initial state
    previous_action = action;
    *out_ << "Human Took Action: " << action << " in the initial state:" << state_->text() << endl;
    *out_ << "Humans new state after init: " << m_new_state << endl;
...
bool Evaluator::RunStep(int step, int round) {
	...
	//leave it true if you want mc sampler to sample new state
  bool auto_sample = true;
  if(auto_sample)   //== using mc simulator ==//
  {
    //m_new_state at first run is set at runinitial websocketclient
    //this is just a safety assignment
    if(stoi(m_new_state) == -1)
    {
        m_new_state = previous_state;
    }
   *out_ << "State is sampled: " << m_new_state << endl;
  }

  else    //== Setting the next state manually ==//
  {
     *out_ << "Please enter the next state: "; //[integer from 0 to 6, press any non-int key for autogen]: ";
     ...
     if (myStream >> m_new_state){
          *out_ << "State is manually entered: " << m_new_state << endl;
     }
     else
     {
  ...
  int new_state = stoi(m_new_state);
	...
	*out_ << "- The real state received: " << m_new_state << endl;
    *out_ << "=== RESULTS ===" << endl;
  ...
  bool terminal = ExecuteAction(sim_type, new_state, previous_action, reward, obs);
	...
	// bool terminal = ExecuteAction(sim_type, m_new_state, previous_action, reward, obs);
    ...
    // terminal = (m_new_state == 1 || m_new_state == 2) ? true : false;
    ...
    // *out_ << "Human Took Action: " << previous_action << " in the state_ state:" << state_->text() << endl;
    // *out_ << "Human Took Action: " << previous_action << " in the new state:" << m_new_state << endl;

    *out_ << "- The state_ text is " << state_->text() << endl;
