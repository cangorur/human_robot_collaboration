...
ValuedAction DESPOT::Search() {
	...
	if (Globals::config.time_per_move <= 0){ // Return a random action if no time is allocated for planning
		cout << "WARNING: Returning a random action !" << endl;
...
void DESPOT::Update(string new_state, int action, OBS_TYPE obs) {
	double start = get_time_second();

	belief_distr = belief_->Update(new_state, action, obs);
	...
	cout << "- Agent's belief from DESPOT:";
	ostringstream oss;

	for (int i = 0; i < belief_distr.size(); i++) {
		pair<string, double> pair = belief_distr[i];
		oss << " " << pair.first << " = " << pair.second << endl;
		cout << oss.str();
	}
}

vector<pair<string, double>> DESPOT::GetBeliefDistribution(void){
	return belief_distr;
}
