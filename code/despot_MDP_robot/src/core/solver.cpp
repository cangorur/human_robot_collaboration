...
void Solver::Update(string new_state, int action, OBS_TYPE obs) {
	double start = get_time_second();

	belief_->Update(new_state, action, obs);
...
std::vector<std::pair<std::string, double>> Solver::GetBeliefDistribution(void){
	std::vector<std::pair<std::string, double>> dummy;
	return dummy;
}
